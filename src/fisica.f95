...
real*8 function Viscosidad_miu(r1, r2, v1, v2, soft_len)
	...
	if(magnitudVector3D(rij) >= soft_len) then
		Viscosidad_miu = productoEscalar3D(vij, rij) / magnitudVector3D(rij)
	else
		!write(*,*) "Use soft len: ", soft_len, " - Separacion: ", magnitudVector3D(rij)
		Viscosidad_miu = productoEscalar3D(vij, rij) / soft_len
	endif
	
	!write(*,*) soft_len, " <- softlen. En Viscosidad_miu. r1, r2: ", r1, r2, " - v1, v2: ", v1, v2, " - Miu: ", productoEscalar3D(vij, rij) / (magnitudVector3D(rij)**2)
	
	!Viscosidad_miu = productoEscalar3D(vij, rij) / (magnitudVector3D(rij)**2)
...
function ArtificialViscosityAcc(idx_particula, temperatura, n_vecinos, lista_vecinos, N, pos_x, pos_y, pos_z, v_x, v_y, v_z, densidades, densidades_locales, masas) result(aceleracion)
	...
	radio1 = calcularRadio(masas(idx_particula), densidades(idx_particula)) / PARSEC_CMS
...
real*8 function ArtificialViscosity(idx_particula, temperatura, n_vecinos, lista_vecinos, N, pos_x, pos_y, pos_z, v_x, v_y, v_z, densidades, densidades_locales, masas)
	...
	integer lista_vecinos(0:n_vecinos-1)		
	real*8 pos_x(0:N-1), pos_y(0:N-1), pos_z(0:N-1), v_x(0:N-1), v_y(0:N-1), v_z(0:N-1), densidades_locales(0:N-1), masas(0:N-1), densidades(0:N-1)
	real*8 r1(0:2), r2(0:2), v1(0:2), v2(0:2)
	real*8 resultado, tmp, densidad_local1, densidad_local2, densidad_local_promedio, temperatura, soft_len, radio1, radio2
	...
	do j = 0, n_vecinos - 1, 1
		...
		tmp = ArtificialViscosityPoints(r1, r2, v1, v2, temperatura, densidad_local1, densidad_local2, soft_len)
		
		!write(*,*) idx_particula, lista_vecinos(j), " ArtificialViscosityPoints: ", tmp
		
		resultado = resultado + (tmp * 0.5D+0 * densidad_local_promedio**2)
	enddo
	
	ArtificialViscosity = resultado / n_vecinos
...
function GradientePresion(idx_particula, n_vecinos, lista_vecinos, presiones, N, pos_x, pos_y, pos_z, masas, densidades) result(vector_gradiente)
...
!y sus vecinos, se calcula el volumen del paralelepípedo que las contiene. El paralelepípedo se define con los valores máximos de x, y, z de los vecinos
...
!	if(soft_len > 0.0D+0) then
!		write(*,*) "Estoy en aceleracion_g_vect, soft_len: ", soft_len/PARSEC_MTS, "Magnitud de R: ", magnitud_pos/PARSEC_MTS, " ratio: ", soft_len/magnitud_pos
!	endif
