...
subroutine pasoLeapFrog(N, itr_inicio, itr_final, Arbol, NodosParticulas, masas, pos_x, pos_y, pos_z, densidades, densidades_locales, v_x, v_y, v_z, acc_x, acc_y, acc_z, dt, umbralBH, tolerancia_colision, beta, n_vecinos, matriz_vecinos, presiones, temperatura, myid)
	...
	integer N, i, j, itr_inicio, itr_final, myid, n_vecinos	
	...
	real*8 dt, umbralBH, dist_max, dist_avg, tolerancia_colision, beta, temperatura, densidad
...
!	write(*,*) myid, "Entrando en pasoLeapFrog. Posiciones(x): ", pos_x, " - \nPosiciones(y): ", pos_y, "- \nPosiciones(z): ", pos_z
	...
	do i = 0, N - 1, 1
		...
		dist_avg = dist_avg + magnitudVector3D(vector_posicion)
	...
	do i = itr_inicio, itr_final, 1
		...
		if(magnitudVector3D(vector_posicion) >= FACTOR_DISTANCIA_MAX * dist_avg) then
			...
			acc_grav_vect(0) = 0.0D+0
			acc_grav_vect(1) = 0.0D+0
			acc_grav_vect(2) = 0.0D+0
			...
			acc_grav_vect = calcularAceleracion(p, Arbol, umbralBH, N)

			!Estimo el gradiente de presión, y la aceleracion

			lista_vecinos = matriz_vecinos(i, 0:n_vecinos-1)

			gradiente_presion = GradientePresion(i, n_vecinos, lista_vecinos, presiones, N, pos_predictor_x, pos_predictor_y, pos_predictor_z, masas, densidades)

			acc_presion_vect = gradiente_presion / (densidades_locales(i) / 1000.0D+0)

			!OJO, aqui ya está tomando en cuenta los componentes de la aceleración por la energía térmica (presión del gas)						

			!OJO 2012-01-08 Cambia suma por resta (aceleracion presion)
	...
	!!		write(*,*) myid, i, " En dinamica.pasoLeapFrog. Particula: ", p%id, " - aceleracion neta sin visc: ", acc_x(i), acc_y(i), acc_z(i), " - aceleracion gravedad: ", acc_grav_vect, magnitudVector3D(acc_grav_vect), " - aceleracion presion: ", acc_presion_vect, magnitudVector3D(acc_presion_vect), " Aceleracion viscosidad: ", acc_visc_vect, magnitudVector3D(acc_visc_vect), " - Gradiente presion: ", magnitudVector3D(gradiente_presion), " - Velocidades: ", v_x(i), v_y(i), v_z(i), " - Densidad(i): ", densidades_locales(i)
			...
			acc_visc_vect = ArtificialViscosityAcc(i, temperatura, n_vecinos, lista_vecinos, N, pos_predictor_x, pos_predictor_y, pos_predictor_z, v_x, v_y, v_z, densidades, densidades_locales, masas)
			...
			if(.false. .and. p%id == 32) then
				write(*,*) myid, p%id, " En dinamica.pasoLeapFrog. Particula: ", p%id, " - Velocidades: ", v_x(i), v_y(i), v_z(i), " - aceleracion gravedad: ", acc_grav_vect, magnitudVector3D(acc_grav_vect), " - aceleracion presion: ", acc_presion_vect, magnitudVector3D(acc_presion_vect), " - aceleracion viscosidad: ", acc_visc_vect, magnitudVector3D(acc_visc_vect), " - Gradiente presion: ", gradiente_presion, magnitudVector3D(gradiente_presion), " - Densidad(i): ", densidades_locales(i)!, " - Vecinos: ", lista_vecinos
			endif
